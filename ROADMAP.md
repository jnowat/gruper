...
- **Remaining open steps:**
  ...
  2. ~~Ollama detection & guidance in the desktop happy path~~ — ✅ partially addressed: the Console's "Add Local Agent" flow (see WP-32 below) probes `http://localhost:11434/api/tags` and pre-fills detected models; there's still no first-run nudge to *install* Ollama if it's entirely absent.
...
#### WP-32.1 — Minimum Viable Agent Onboarding ("Add Local Agent") — 🟢 implemented and verified end-to-end on Linux
...
- **`console/src-tauri/src/lib.rs`**: new `spawn_local_agent` Tauri command. Spawns the bundled `gruper-agent` sidecar (new `bundle.externalBin` entry, staged by `scripts/build-desktop.sh`/`.ps1` alongside the orchestrator) with `AGENT_ID`/`JWT_TOKEN`/`ORCHESTRATOR_URL` (converted from the Console's http(s) URL to the agent's expected `ws(s)://.../v1/agents/ws`)/`OLLAMA_URL` env vars. Each spawned agent gets its own working directory under the Tauri app-data dir so multiple local agents' SQLite offline queues (`agent.db`) don't collide. `SidecarState` was generalized from "one orchestrator child" to "one orchestrator child + a map of agent children by id" so every spawned process — not just the orchestrator — gets killed on Console exit.
- **`console/src/lib/components/AddAgentDialog.svelte`** (new) + a "+ Add" button in the fleet sidebar: generates a fresh agent identity the same way the Console generates its own (`generateRandomPubkey`, factored out of `stores/auth.ts`), probes `http://localhost:11434/api/tags` for installed Ollama models (falls back to a placeholder model tag if none found or Ollama isn't running, since the orchestrator's schema requires at least one), calls `POST /v1/agents` with the **Console's own JWT** — verified correct because gd-0.1 tokens are per-owner, not per-agent (`orchestrator/ws/agent_ws.py` only checks the token's `sub` against the agent's `owner_id`) — then invokes `spawn_local_agent`. The new agent is pushed into `fleetStore` immediately (new `fleetStore.add()`) so it's visible before its first heartbeat, rather than waiting on the next full REST reload.
...
- **Verified end-to-end on Linux** (not just code-reviewed): stood up a real orchestrator, registered a real agent via the same REST calls the dialog makes, ran `agent-runtime/main.py` against it with `GRUPER_EXIT_WITH_PARENT=1`, confirmed it registered over the WebSocket and the orchestrator's `GET /v1/agents` reported it `idle`. Separately verified the orphan watchdog: spawned the agent as a child of a throwaway parent process, `SIGKILL`'d the parent, confirmed the agent process exited on its own within one poll interval.
...
- **⚠️ Honest caveats:**
  - **Not run on Windows.** Same caveat as the rest of WP-31/32 — the sidecar-spawn code path and the `agent-runtime` fixes above were written carefully for Windows (the signal-handler fix specifically targets a Windows-only failure mode) but have not been exercised on physical Windows hardware.
  - **No "install Ollama for me" flow.** If Ollama isn't running, the dialog registers the agent with a placeholder model tag it can't actually serve until the user installs Ollama and pulls that model themselves — it doesn't guide them through that.
...
*Last updated: 2026-07-01 (Execution pass: fixed the sidecar's working-directory assumption — orchestrator.db/.gruper_jwt_secret now resolve against the Tauri app-data directory instead of an arbitrary CWD, which was a real crash risk on a Windows installer launch (Program Files is not user-writable); fixed the broken `orchestrator/README.md` Quick Start command (verified the old one failed with a Python relative-import error); corrected several stale "Windows CI has never succeeded" claims — it has, 18 times, including on `main`, though a human still hasn't run the resulting installer on physical hardware; and added the minimum-viable "Add Local Agent" flow — the Console can now generate an agent identity, detect local Ollama models, register the agent with the orchestrator, and spawn it as a second sidecar, end to end, without the user touching a config file. WP-31/WP-32's core packaging and auto-run work (PyInstaller executables, JWT auto-generation, sidecar auto-connect, forceful-kill orphan-detection watchdog) still stands from the prior session, Linux-validated. Real Windows hardware validation remains the single largest open gate on the desktop-first push.)*
