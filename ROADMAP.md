...
**Status as of 2026-07-01:** `gd-0.1` / `gd-0.2` / `gd-0.2.x` — **Phase 0 complete; WP-03/04/05 code complete; WP-06 automated end-to-end relay validation is green (17/17 — five dispatch-contract bugs found and fixed); WP-30 (SQLite backend) is complete — the orchestrator now runs on SQLite by default, with PostgreSQL as an opt-in server tier.** Remaining gates on the desktop-first push:
...
- **WP-31** (one-click Windows packaging) and **WP-32** (orchestrator auto-run) are not started — today a desktop user still runs `pip install` + `uvicorn` by hand and starts the orchestrator separately from the Console.

· WP-01 ✅ · WP-02 ✅ · WP-03 ✅ code complete · WP-04 ✅ code complete · WP-05 ✅ complete · WP-06 ✅ automated E2E validated on **both backends** (17/17 SQLite; 17/17 PostgreSQL; SC-2 ~10 ms on either) · 🔲 real two-machine NAT field run pending · **WP-30 ✅ complete** (SQLite default, PostgreSQL opt-in, dual CI) · 🔲 WP-31/WP-32 not started · OQ-1 and OQ-2 resolved · **v1.0 is a future finish line gated on SC-1…SC-9; it has not been reached.**

---

## ⚠️ Honest current state (read this first)

**Updated 2026-07-01 — WP-30 landed.** The gap called out in the previous revision of this section (orchestrator hard-wired to PostgreSQL) is closed. Be precise about what's true now vs. what's still open:
...
| **Orchestrator** | ✅ **Yes, as of WP-30** — `pip install -r requirements.txt && uvicorn orchestrator.main:app`, no `DATABASE_URL` needed | **SQLite by default** (`orchestrator.db`); **PostgreSQL opt-in** via `DATABASE_URL=postgresql://...` |

**What changed in WP-30:** a backend abstraction (`orchestrator/db/`) now sits between the routers/dispatcher and the driver. `DATABASE_URL` defaults to `sqlite:///orchestrator.db`; setting it to a `postgresql://` DSN switches to the PostgreSQL/server tier with identical behavior. Both backends pass the identical `pytest` suite (35/35) and the WP-06 E2E harness (17/17), verified directly: `DATABASE_URL` unset → real `uvicorn` process starts with **no Docker, no PostgreSQL**, migrates a fresh `orchestrator.db`, and serves `GET /v1/health` → `{"status":"ok","db":"ok"}`.

**What is still honestly incomplete (do not overclaim this either):**
- **No packaging.** A desktop user still needs Python + `pip install` + a manual `uvicorn` invocation — there's no installer, no bundled executable. That's WP-31.
- **No auto-start.** The Console does not launch the orchestrator; a user runs it as a separate step. That's WP-32.
...
| **1.5 — Desktop-First Foundation** | **`gd-0.2.x`** | **SQLite-default orchestrator; whole stack on a Windows desktop, no Docker; orchestrator auto-run** | **🔄 WP-30 ✅ complete; WP-31/WP-32 🔲 not started — current priority** |
...
### WP-31 — One-Click Local Setup & Packaging (Windows-first) — 🔲 `gd-0.2.x` · not started, feasibility spike done
...
- **Feasibility spike done (2026-07-01, this session — Linux, de-risks the approach but is not the Windows build itself):** PyInstaller `--onefile` successfully bundles the orchestrator into a single ~15–29 MB self-contained executable with **no Python installation, no Docker, no PostgreSQL** on the target — built, ran, passed `GET /v1/health`, and was confirmed **relocatable** (copied to an unrelated directory, still ran and created its SQLite file correctly there). Two non-obvious things had to be fixed to get this working, worth knowing before Windows packaging starts:
  1. The ASGI app must be **imported as an object** in the entry script (`from orchestrator.main import app; uvicorn.run(app, ...)`), not passed as the string `"orchestrator.main:app"` — PyInstaller's frozen import system can't resolve a dynamic string-based import target, so `uvicorn.run("orchestrator.main:app", ...)` fails at runtime with `Could not import module`.
  2. PyInstaller needs `--paths <repo-root>` to discover the local `orchestrator` package at all, since the entry script lives outside the package directory.
  This was Linux-only (PyInstaller does not cross-compile — a Windows `.exe` must be built ON a Windows runner, same pattern already established for the Tauri console build in `build-windows.yml`). The actual Windows build, and bundling `agent-runtime` the same way, are still open work.
- **Steps:**
  1. **Bundle the orchestrator + agent Python runtimes** — PyInstaller `--onefile`, per the spike above, run on a `windows-latest` GitHub Actions runner (mirroring `build-windows.yml`'s existing pattern) to produce the actual `.exe`. Bundle `agent-runtime` the same way. SQLite is embedded; nothing external to install. **Open question to resolve during implementation:** whether to trim the `asyncpg` dependency (see Known Technical Debt) before bundling, since it adds real size/build time to a SQLite-only executable for no functional benefit.
  ...
  6. **Docs restructure:** README leads with the **desktop (SQLite, no-Docker) quick start**; the Docker Compose + PostgreSQL instructions move under an "Advanced / Server deployment" heading. (WP-30 already did a light version of this in `orchestrator/README.md`; this step is the fuller root-`README.md` restructure.)
  7. **Desktop happy-path validation:** run the WP-06 E2E flow (already `--backend`-selectable as of WP-30) on a **real Windows desktop** with SQLite, no Docker, and a **real Ollama** (feeds the still-open half of the SC-8 gate — see WP-06's notes).
- **Files:** packaging config (e.g. `agent-runtime/`, `orchestrator/` build specs), installer/scripts, `.github/workflows/` (new Windows packaging job), `README.md` (desktop-first restructure), `orchestrator/README.md`, `agent-runtime/README.md`, Windows service docs
- **Exit gate:** On a clean Windows machine with only Ollama installed, a user runs one installer/command and reaches a working local stack (orchestrator + agent registered + Console connected) — **no Docker, no PostgreSQL, no manual Python setup** — and completes a task. SC-8 demonstrated end-to-end (both halves: SQLite-only *and* real Windows).
...
- **Depends on:** WP-30 (SQLite default) — ✅ done; WP-31 (bundled runtime) — the PyInstaller executable WP-31's spike validated is exactly what a Tauri sidecar needs to spawn (a single native binary, no Python runtime dependency on the target machine).
...
| **`asyncpg` is a hard dependency even for pure-SQLite runs** | **Medium** (new, found during WP-30) | `orchestrator/db/connect.py` imports `PostgresDatabase` unconditionally, which imports `asyncpg` at module load — so `pip install` for a desktop-only setup still pulls in the Postgres driver. Not a functional blocker (SQLite works fine regardless), but works against "minimal extra dependencies." Candidate fix in WP-31: lazy-import `asyncpg` only when a `postgresql://` URL is actually used, or split `requirements.txt` into desktop/server variants. |
...
| **No packaging/installer for the orchestrator or agent** | **Medium** | Desktop setup today is still `pip install` + a manual `uvicorn`/`python main.py` invocation — no bundled executable, no installer. Closed by WP-31. |
...
*Last updated: 2026-07-01 (WP-30 complete: the orchestrator now runs on SQLite by default with no Docker/PostgreSQL required, verified directly — server starts, full 35-test suite, and the WP-06 17-check E2E harness all green on both SQLite and PostgreSQL; new `orchestrator/db/` backend abstraction; a genuine pre-existing WS-disconnect cleanup bug and two dependency-drift test failures were found and fixed along the way, confirmed via a clean pre-WP-30 checkout not to be WP-30 regressions; WP-31 one-click packaging and WP-32 orchestrator auto-run remain open, current priority)*
