...
**Status as of 2026-07-01 (end of day):** `gd-0.1` / `gd-0.2` / `gd-0.2.x` — **Phase 0 complete; WP-03/04/05 code complete; WP-06 automated end-to-end relay validation is green (17/17 — five dispatch-contract bugs found and fixed); WP-30 (SQLite backend) is complete; WP-31 (desktop packaging) and WP-32 (orchestrator auto-run) are code-complete and validated on Linux, including against a real production Tauri build; WP-32.1 (agent onboarding) has been through three rounds of real-Windows bug fixes; WP-32.2 (debug logging, model selection, master/detail result view) is complete — Windows hardware validation is the one gate left across WP-31/32/32.1.** A same-day Phase 2 plan ([`docs/Phase2-gd-0.3-Plan.md`](docs/Phase2-gd-0.3-Plan.md)) lays out WP-07…WP-11 in concrete, codebase-grounded detail; none of that Phase 2 code has started yet — WP-32.2 was the "harden the desktop first" work the plan itself recommended doing before it. Remaining gates on the desktop-first push:
...
- **WP-31/WP-32/WP-32.1 have not been re-verified on real Windows hardware after their latest fixes.** Everything was built and verified on Linux. The Windows CI job (`build-windows.yml`) itself **has succeeded repeatedly** — 18 green runs as of this writing, including on `main` — so it reliably produces `.exe`/`.msi`/agent-orchestrator artifacts; what's still open is a human downloading and running one of those artifacts on a physical Windows machine, and the `.ps1` script is likewise unverified there. This is now the single biggest remaining risk on the desktop-first push — see each WP's own section for specifics.

· WP-01 ✅ · WP-02 ✅ · WP-03 ✅ code complete · WP-04 ✅ code complete · WP-05 ✅ complete · WP-06 ✅ automated E2E validated on **both backends** (17/17 SQLite; 17/17 PostgreSQL; SC-2 ~10 ms on either) · 🔲 real two-machine NAT field run pending · **WP-30 ✅ complete** (SQLite default, PostgreSQL opt-in, dual CI) · **WP-31 🟡 Linux-validated, Windows CI green (18 runs) but installers not yet run on real hardware** · **WP-32 🟡 sidecar auto-connect implemented and validated on Linux against a real `tauri build` production binary; agent onboarding flow has been through three real-Windows-testing rounds so far, each surfacing and fixing a genuine bug — placeholder-model registration, a Private Network Access-blocked Ollama probe, and (most recently) an indefinitely-hanging "waiting for agent" step plus a white-on-white dropdown — none yet re-verified on Windows hardware after their fixes** · **WP-32.2 ✅ complete** (unified debug logging, explicit default-model selection, master/detail result view) · OQ-1 and OQ-2 resolved · **v1.0 is a future finish line gated on SC-1…SC-9; it has not been reached.**
...
| **1.5 — Desktop-First Foundation** | **`gd-0.2.x`** | **SQLite-default orchestrator; whole stack on a Windows desktop, no Docker; orchestrator auto-run; agent onboarding; debug logging/model selection/result view** | **🔄 WP-30 ✅ · WP-31 🟡 · WP-32 🟢 · WP-32.1 🟡 · WP-32.2 ✅ — all code-complete, Linux-validated — Windows hardware validation is the last gate across WP-31/32/32.1** |
...
#### WP-32.1 — Minimum Viable Agent Onboarding ("Add Local Agent") — 🟡 implemented and verified end-to-end on Linux; two rounds of real-Windows testing have each found and fixed a genuine bug, re-verification on Windows still pending
...
- **Third round of real-Windows testing (after the PNA fix): Ollama detection now works (all 6 real models shown), but clicking "Add Agent" left the dialog stuck on "Waiting for agent to connect" forever, with a fleet entry appearing but the agent never becoming usable, plus a separate UI bug (Role Template dropdown: white text on white background until hovered) and two UX gaps (no way to clear stale tasks; clicking an agent gave no useful action beyond selecting it).** Fixed all four this pass:
  1. **The waiting logic was rewritten from a hand-rolled `Promise`/event-listener graph to a plain bounded polling `while` loop** in `AddAgentDialog.svelte`'s `waitForAgentOnline()`. The previous version resolved via a `fleetStore` subscription callback, a Tauri `agent-sidecar-exited` event listener, and a `setTimeout`, all coordinated through manually-tracked cleanup closures — plausible on inspection, but with enough moving parts that a subtle timing bug was hard to rule out, and a real Windows run showed it getting stuck. The `agents` table's `status` column defaults to `'offline'` (checked directly in the migration, not assumed), which rules out the most obvious race — a synchronous "already online" callback firing before its own cleanup closure existed — but does **not** rule out the wait genuinely depending on a single point of failure: an orchestrator `fleet_event` WebSocket push that, for whatever exact reason, wasn't reliably reaching this dialog. The new version polls THREE independent signals every 1.5 s inside a loop whose exit condition is wall-clock time (`Date.now() < deadline`) — a crash event, the local `fleetStore` (fast path if the WS push does arrive), and a **direct `GET /v1/agents` REST re-fetch** (authoritative, and self-healing regardless of whether the WS push ever arrives at all). The `await waitForAgentOnline(...)` call site is now wrapped in `try`/`catch` so literally nothing left in that function can freeze `step` at `'waiting'` again, and a "Stop waiting and close" button gives the user a manual way out at any time, on top of the (now genuinely reliable) automatic path.
  2. **Role Template dropdown contrast** — Chromium/WebView2's native `<select>` popup doesn't inherit a page's translucent Tailwind background (`bg-white/5`) the way the closed control does, so every `<option>` rendered with a default white popup background while still inheriting `text-white`, unreadable until the OS's hover-highlight color took over. Fixed by setting an explicit solid `bg-slate-800 text-white` directly on every `<option>` in both `AddAgentDialog.svelte` and `TaskComposer.svelte` (Role Template and Data Class) — the same underlying bug would have hit those too.
  3. **Stale tasks** — `tasksStore` gained `clearFailed()`/`clearAll()` (session-only; there is no backend DELETE endpoint for tasks, so this doesn't touch the orchestrator's own records) and the Tasks panel in `+page.svelte` shows "Clear failed"/"Clear all" once there's anything to clear.
  4. **Agent management** — a new `stop_local_agent` Tauri command (`console/src-tauri/src/lib.rs`) stops an agent this Console spawned locally (kills the tracked child, removes it from `SidecarState.agents`) and a small "✕" button appears on hover on each `AgentCard`, wired to it. There's still no orchestrator-side delete, so the frontend marks the row `offline` itself (`fleetStore.setOffline`) after a successful stop; the orchestrator converges to the same state on its own once it notices the WebSocket drop. Stopping an agent this Console did **not** spawn (remote/manually-run) correctly surfaces a plain "not managed by this Console" message rather than pretending to succeed.
  All four are covered by `cargo check`/`cargo test` (still 9/9 — no test changes were needed for `stop_local_agent`, which is a thin wrapper around already-tested `SidecarState` bookkeeping) and `svelte-check`, both clean. None of this has been re-run on physical Windows hardware yet.
...
- **⚠️ Honest caveats:**
  ...
  - ~~No "remove/stop agent" UI yet~~ — ✅ addressed this pass: a per-agent "✕" stop action now exists (see above). Still not a full lifecycle UI (no rename, no restart-in-place, no bulk actions) — good enough for "stop the one I just added by mistake," not a fleet management console.
...
- **Files:** `console/src-tauri/src/lib.rs` (`spawn_local_agent`, `detect_ollama_models`, `stop_local_agent`, `SidecarState` generalized), `console/src-tauri/Cargo.toml` (tokio `macros` feature + dev-only `rt`), `console/src-tauri/tauri.conf.json` (`bundle.externalBin` +`binaries/gruper-agent`), `console/src/lib/components/AddAgentDialog.svelte` (new), `console/src/lib/components/AgentCard.svelte` (+stop/remove action), `console/src/lib/components/TaskComposer.svelte` (`<option>` contrast fix), `console/src/routes/+page.svelte` (+"Add" button, agent removal wiring, task-clearing buttons), `console/src/lib/stores/auth.ts` (`generateRandomPubkey` factored out), `console/src/lib/stores/fleet.ts` (+`add()`, +`setOffline()`), `console/src/lib/stores/tasks.ts` (+`clearFailed()`, +`clearAll()`), `console/src/lib/api/client.ts` (+`registerAgent`), `console/src/lib/types.ts` (+`AgentRegistrationRequest`), `agent-runtime/main.py` (signal-handler guard + orphan watchdog + standalone-run help message), `agent-runtime/ws_client.py` (configured-capabilities model fallback), `agent-runtime/requirements.txt` (+`psutil`), `scripts/build-desktop.sh`/`.ps1` (stage both sidecars).
...
#### WP-32.2 — Desktop Hardening Pass (Debug Logging, Model Selection, Result View) — ✅ complete · `gd-0.2.x`, ahead of `gd-0.3`

- **Goal:** per `docs/Phase2-gd-0.3-Plan.md` §1's assessment ("harden the desktop tier first, 2–3 weeks, before a narrow Phase 2 MVP"), close the three desktop-usability gaps a real user would hit before any cross-network sharing lands.
- **What shipped (2026-07-01):**
  1. **Unified, cross-tier debug logging.** A single Rust in-memory ring buffer (5,000 entries) in the Console is now the sink for all four processes — Python orchestrator and agent sidecars (`structured_log.py`: one sentinel-prefixed JSON line per record on stdout), the Rust/Tauri layer itself (`rust_log()` — previously invisible `eprintln!` for spawn/health/crash decisions), and the Svelte frontend (`stores/logs.ts`, plus a `console.*` bridge). A new **Debug panel** (🐞 button / `Ctrl`/`Cmd`+`` ` ``) filters by category and level, searches on message text or the promoted `agent_id`/`task_id` (so one id gives the full cross-tier trace of a registration or dispatch), and supports live tail, copy, `.jsonl`/`.txt` export, and a one-click "Copy diagnostics" bundle for bug reports. Redaction runs twice (Python emit-time, Rust sink-time) against JWTs, `?token=` URLs, and pubkey/secret-shaped fields. Full detail: `docs/Debug-Logging.md`.
  2. **Explicit default-model selection.** `capabilities.default_model` is now a first-class field (`types.ts`, orchestrator `AgentCapabilities`, `agent.schema.json`); `ws_client._model_and_options()` honors it and falls through cleanly on an empty value instead of ever silently hardcoding a model. `AddAgentDialog` gets an explicit "Default model" picker after Ollama detection; `TaskComposer`'s free-text model field became a `<select>` of the agent's own models plus "Agent default." Closes the "detected model never reaches the running agent" class of bug found during WP-32.1.
  3. **Master/detail result view.** `+page.svelte` reworked from a cramped 28rem right-rail "floating box" to a three-pane Fleet | Tasks | wide Detail layout; task composition moved to a modal ("+ New task") so the result pane gets the space. `ResultView` now renders real `marked` + DOMPurify markdown (statically bundled, sanitized, plaintext fallback) with a copy-result action and a fetch-error state distinct from "still running."
- **Files:** `orchestrator/structured_log.py`, `agent-runtime/structured_log.py` (new), `console/src-tauri/src/lib.rs` (`LogEntry`, ring buffer, `ingest_sidecar_line`, `rust_log`, `get_logs`/`push_log`/`clear_logs`), `console/src/lib/stores/logs.ts` (new), `console/src/lib/components/DebugPanel.svelte` (new), `console/src/lib/types.ts`, `console/src/lib/components/{AddAgentDialog,TaskComposer,AgentCard}.svelte`, `agent-runtime/ws_client.py`, `orchestrator/agent.schema.json` (or equivalent capability schema), `console/src/routes/+page.svelte`, `console/src/lib/components/ResultView.svelte`, `docs/Debug-Logging.md` (new), `docs/Phase2-gd-0.3-Plan.md` (new — the plan this pass executes against).
- **Exit gate:** met — verified via `cargo check`/`cargo test` and `svelte-check` clean; not yet exercised on real Windows hardware (same standing caveat as WP-31/32/32.1).
- **Not yet done:** the Phase 2 (`gd-0.3`) work packets themselves (WP-07…WP-11) below — this pass is preparatory hardening, not sharing functionality.
...
| ~~Companion spec not yet desktop-first~~ | ✅ **Resolved (2026-07-01 doc pass)** | `GruperDistributedSpec.md` §5.1 (recommended stack) and §9 (data models) now lead with SQLite-by-default / PostgreSQL-opt-in, matching this roadmap's two-tier framing. §10's technology table updated to match. |
...
*Last updated: 2026-07-01 (Three execution passes today. First pass hardened the "Add Local Agent" flow end to end: Ollama detection became a real state machine (unreachable / no models / ready / error) gating the "Add Agent" button so it no longer silently registers a placeholder-model agent; fixed a root-cause bug where the detected model never reached the spawned agent process (`spawn_local_agent` didn't set `CAPABILITIES`, and `agent-runtime/ws_client.py` hardcoded `"llama3.1:8b"` regardless); `spawn_local_agent` gained a crash-detection grace period and an exit event instead of reporting false success; `agent-runtime/main.py`'s standalone-run error was replaced with plain-language guidance toward the Console; fleet's empty state now explains the Ollama prerequisite. **Second pass, triggered by an actual Windows hardware test report:** that test found the "Add Local Agent" dialog still reported "Ollama is not running" against a machine where Ollama demonstrably was running with multiple models. Root cause tracked down and confirmed against Tauri/Chromium's documented behavior: the dialog's Ollama probe was a plain frontend `fetch()`, and Chromium/WebView2's Private Network Access policy silently blocks exactly that kind of request (webview origin → loopback service) unless the target sends an `Access-Control-Allow-Private-Network` header, which Ollama's server never does — indistinguishable from "not running" at the JS layer, and impossible to catch by testing only on Linux or against a plain browser tab (both of which are themselves `localhost`-hosted and never trigger the policy). Fixed by moving the actual HTTP call out of the webview into a new `detect_ollama_models` Rust command that talks a raw socket directly, with 9 new Rust tests (including a real loopback-socket end-to-end test) backing it. **Third pass, from a follow-up test report on that same fix:** Ollama detection now worked (all 6 real models shown), but clicking "Add Agent" left the dialog stuck on "Waiting for agent to connect" indefinitely — a fleet entry appeared but the agent never became usable — plus a Role Template dropdown with unreadable white-on-white text until hovered, and no way to clear stale tasks or do anything with a fleet agent beyond selecting it. The waiting logic was rewritten from a hand-rolled Promise/event-listener graph (fleetStore subscription + Tauri crash-event listener + setTimeout, coordinated through manual cleanup closures) to a plain bounded polling loop that checks three independent signals — a crash event, the local fleetStore, and a direct authoritative `GET /v1/agents` REST re-fetch — every 1.5 s against a hard wall-clock deadline, wrapped in try/catch at the call site, plus a manual "Stop waiting and close" escape hatch; the dropdown bug was fixed by setting an explicit solid background/text color directly on every `<option>` element (native select popups don't inherit a page's translucent background); tasks gained session-scoped "Clear failed"/"Clear all" buttons; and agents gained a per-agent stop/remove action backed by a new `stop_local_agent` Tauri command. Real Windows hardware re-verification is now the single most important open item across all three passes — everything else that says "Linux-validated" in this document carries the same "not proven on Windows" caveat these bugs keep demonstrating concretely.)*
*Companion document: `GruperDistributedSpec.md` — architecture diagrams, data models, wire schemas, security threat table, and open questions (OQ-1…OQ-5). Desktop-first alignment pass complete (2026-07-01): §5.1 and §9 now lead with SQLite-by-default / PostgreSQL-opt-in, matching this roadmap.*
