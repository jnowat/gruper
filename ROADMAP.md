...
- **Steps:**
  1. ✅ `docker-compose.yml`: PostgreSQL 16 + FastAPI with hot reload; all config via environment variables.
...
- **Steps:**
  ...
  2. ✅ WSS client to orchestrator (`ConsoleWS` class with exponential-backoff reconnect — Gruper core's retry discipline); orchestrator `GET /v1/console/ws` implemented.
  3. ✅ **Fleet view:** `AgentCard` — name, status badge (idle / busy / offline / degraded / draining), last-seen relative time, model list; updates live from `fleet_event` WS pushes.
  4. ✅ **Task composer:** full task input + Gruper core's inference parameters (temperature, top-p, top-k, repeat penalty, max tokens, context length) + role template + data class + priority; mirrors core's task-input UX conventions exactly.
  ...
  8. ✅ CI build: `console/package-lock.json` committed and **verified `npm ci`-coherent** (regenerated with npm 11 to dodge npm 10.9's lockfile-generation bug; npm 10 *and* npm 11 both accept it). Readiness check passes, so `tauri-action` runs the real NSIS/WiX build on push to `main`, on `v*` tags, on `claude/**` feature-branch pushes (development branches produce artifacts without needing a PR), on PRs into `main`, and on manual dispatch. Workflow pins npm to 11, logs the full toolchain (node/npm/rust/cargo/`tauri info`) before building, and **always leaves a downloadable artifact** — the installers on success, a `BUILD-DIAGNOSTICS.txt` on a ready-but-failed build, or a `BUILD-STATUS.txt` placeholder when the console is not yet scaffolded. Frontend (`npm ci && npm run build && npm run check`) verified green on Linux. **Windows build fix:** the Tauri v2 library-naming bug (`gruper_console_lib` unresolved — `Cargo.toml` had no `[lib]` section, so the library defaulted to the package/bin name `gruper_console`) is fixed with an explicit `[lib] name = "gruper_console_lib"` (`crate-type = ["staticlib", "cdylib", "rlib"]`); the placeholder bundle icons (RGB PNGs that made `tauri::generate_context!` panic with *"icon … is not RGBA"*) are regenerated as valid RGBA via `tauri icon`. With both fixes the full native Tauri shell now `cargo build`s green on Linux (deps + `lib.rs`/`main.rs` link + `generate_context!` asset validation all pass); the Windows MSVC bundle leg (`.exe`/`.msi`) runs on the GitHub Windows runner. `Cargo.lock` committed for reproducible CI builds.
  9. 🔲 Playwright click-through smoke test — deferred to the `gd-0.6+` integration-test suite (non-blocking; tracked in Known Technical Debt).
- **Files:** `console/src-tauri/tauri.conf.json`, `console/src-tauri/Cargo.toml`, `console/src-tauri/src/main.rs`, `console/src-tauri/src/lib.rs`, `console/src-tauri/capabilities/default.json`, `console/src-tauri/Cargo.lock` (committed for reproducible builds), `console/src-tauri/icons/` (RGBA set: 32x32, 128x128, 128x128@2x, icon.ico, icon.icns, icon.png), `console/package.json`, `console/svelte.config.js`, `console/vite.config.ts`, `console/tailwind.config.js`, `console/postcss.config.js`, `console/tsconfig.json`, `console/src/app.html`, `console/src/app.css`, `console/src/routes/+layout.ts`, `console/src/routes/+layout.svelte`, `console/src/routes/+page.svelte`, `console/src/lib/types.ts`, `console/src/lib/api/client.ts`, `console/src/lib/stores/auth.ts`, `console/src/lib/stores/fleet.ts`, `console/src/lib/stores/tasks.ts`, `console/src/lib/ws/console_ws.ts`, `console/src/lib/components/ConnectDialog.svelte`, `console/src/lib/components/AgentCard.svelte`, `console/src/lib/components/TaskComposer.svelte`, `console/src/lib/components/ResultView.svelte`, `console/src/lib/components/AgentAnalytics.svelte`; `orchestrator/ws/console_ws.py` (new), `orchestrator/connection_manager.py` (console tracking added), `orchestrator/ws/agent_ws.py` (relay wired), `orchestrator/main.py` (`/v1/console/ws` mounted)
- **Exit gate:** Console builds on Linux (`npm ci && npm run build` **and** `cargo build` in `src-tauri/` both verified green); submits a task; displays result in the embedded Gruper core result view. Windows installer build armed — the lib-naming + RGBA-icon fixes make the Tauri shell compile and bundle, and the CI Windows runner produces the `.exe`/`.msi`.

**Notes (2026-06-29):** Scaffold complete and **verified green on Linux** — `npm ci`, `npm run build`, and `npm run check` (0 type errors). `package-lock.json` committed (198 packages, lockfileVersion 3). Key decisions: `ConsoleWS` reconnects with Gruper core's `[2, 4, 8, 16]s` backoff; `fleet_snapshot` message sent on WS connect so console avoids an extra REST round-trip; `auth.ts` persists JWT in `localStorage` (survives Tauri restarts); inference parameters in `TaskComposer` match Gruper core's ranges exactly (temperature 0-1, top-p 0-1, top-k 1-100, repeat-penalty 0.5-2, max-tokens 128-8192, context-length 512-16384); Chart.js dynamically imported to keep initial bundle small; CSP locked to `connect-src 'self' ws: wss: http: https:` from the first commit; `vite` pinned to `^6` to satisfy `@sveltejs/vite-plugin-svelte@5`'s peer requirement; `tsconfig.json` extends the SvelteKit-generated config so `$app`/`$lib` aliases resolve. **Lockfile fix:** npm 10.9's `npm install` emits an internally inconsistent `picomatch` entry that breaks `npm ci`; regenerated the lock with npm 11 (accepted by both npm 10 and npm 11 `ci`) and the workflow now pins npm to 11. **Windows build fix (2026-06-29):** two blockers that broke `npm run tauri build -- --target x86_64-pc-windows-msvc --bundles nsis,msi` are resolved — (1) `Cargo.toml` had no `[lib]` section, so the library crate defaulted to `gruper_console` while `main.rs` called `gruper_console_lib::run()` (`error[E0433]: unresolved … gruper_console_lib`); added an explicit `[lib] name = "gruper_console_lib"` (the conventional Tauri v2 `_lib` suffix that also avoids the Windows bin/lib name collision, rust-lang/cargo#8519); (2) the placeholder icons were RGB PNGs, which made `tauri::generate_context!` panic (*"icon … is not RGBA"*) — regenerated the full icon set as valid RGBA with `tauri icon`. The whole native Tauri shell now `cargo build`s green on Linux (verified end-to-end, including `generate_context!`), and `Cargo.lock` is committed for reproducibility. **Pending (non-blocking):** the actual Windows MSVC `.exe`/`.msi` bundle runs on the GitHub Windows runner (not cross-buildable from this Linux host); Playwright click-through (deferred to `gd-0.6+`); macOS `.dmg` still needs a macOS runner job (the `.icns` icon is now generated). **Complete.**
