...
· WP-01 ✅ · WP-02 ✅ · WP-03 ✅ code complete · WP-04 ✅ code complete · WP-05 ✅ complete · WP-06 ✅ automated E2E validated on **both backends** (17/17 SQLite; 17/17 PostgreSQL; SC-2 ~10 ms on either) · 🔲 real two-machine NAT field run pending · **WP-30 ✅ complete** (SQLite default, PostgreSQL opt-in, dual CI) · **WP-31 🟡 Linux-validated, Windows CI green (18 runs) but installers not yet run on real hardware** · **WP-32 🟢 sidecar auto-connect implemented and validated on Linux against a real `tauri build` production binary, Windows unverified; agent onboarding flow hardened this pass — no more placeholder-model registration, detected models now actually reach the spawned agent, and spawn failures are reported instead of silently timing out** · OQ-1 and OQ-2 resolved · **v1.0 is a future finish line gated on SC-1…SC-9; it has not been reached.**
...
- **⚠️ Honest caveats:**
  - **Not run on Windows.** Same caveat as the rest of WP-31/32 — the sidecar-spawn code path (including this pass's grace-period crash detection) and the `agent-runtime` fixes above were written carefully for Windows (the signal-handler fix specifically targets a Windows-only failure mode; the standalone-exe pause specifically targets the double-click-closes-the-window problem) but have not been exercised on physical Windows hardware.
...
*Last updated: 2026-07-01 (Execution pass: hardened the "Add Local Agent" flow end to end. Ollama detection is now a real state machine (unreachable / no models / ready / error) that runs automatically when the dialog opens and gates the "Add Agent" button — it no longer silently registers a placeholder-model agent that can never run a task. Fixed a root-cause bug where the detected model never reached the spawned agent process at all (`spawn_local_agent` didn't set a `CAPABILITIES` env var, and `agent-runtime/ws_client.py` hardcoded `"llama3.1:8b"` as the task-execution fallback regardless); both are now wired together. `spawn_local_agent` now catches an immediate sidecar crash within a short grace period and returns a real error instead of reporting false success, and emits an event on any later crash so the dialog can report it instead of a blind "should appear online soon." The dialog itself now waits for the new agent to actually show up online (or reports why it didn't) before declaring success. `agent-runtime/main.py`'s standalone-run error (previously a raw `curl` one-liner aimed at a developer) is replaced with plain-language guidance pointing to the Console's "+ Add Local Agent" button, with a pause so a Windows double-click launch's console window doesn't close before it can be read. Fleet's empty state now explains the Ollama prerequisite and highlights the "+ Add" button. Prior pass's fixes (sidecar working-directory resolution, `orchestrator/README.md` Quick Start, Windows CI status corrections) still stand. Real Windows hardware validation remains the single largest open gate on the desktop-first push.)*
