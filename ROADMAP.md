...
- **WP-31/WP-32 have not been run on real Windows hardware.** Everything was built and verified on Linux this session (no Windows machine available); the Windows CI job was written but not watched succeed, and the `.ps1` script is unverified. This is now the single biggest remaining risk on the desktop-first push — see each WP's own section for specifics.

· WP-01 ✅ · WP-02 ✅ · WP-03 ✅ code complete · WP-04 ✅ code complete · WP-05 ✅ complete · WP-06 ✅ automated E2E validated on **both backends** (17/17 SQLite; 17/17 PostgreSQL; SC-2 ~10 ms on either) · 🔲 real two-machine NAT field run pending · **WP-30 ✅ complete** (SQLite default, PostgreSQL opt-in, dual CI) · **WP-31 🟡 Linux-validated, Windows CI added but unverified on real hardware** · **WP-32 🟢 sidecar auto-connect implemented and validated on Linux against a real `tauri build` production binary, Windows unverified** · OQ-1 and OQ-2 resolved · **v1.0 is a future finish line gated on SC-1…SC-9; it has not been reached.**
...
| **Agent runtime** | ✅ Yes on Linux, packaged as a self-contained executable (`agent-runtime/packaging/`) — Windows `.exe` produced by CI, not yet run on real Windows | **SQLite** (`agent.db`, local offline queue) |
| **Manager Console** (Tauri) | ✅ Yes — native desktop app; **auto-starts and auto-connects to a local orchestrator with zero manual steps** (WP-32), verified against a real production build on Linux | `localStorage` / tauri-store (auth token); pubkey identity auto-generated, no manual command |
| **Orchestrator** | ✅ Yes on Linux, packaged as a self-contained executable (`orchestrator/packaging/`), zero-config (auto-generates JWT secret, auto-creates SQLite file) — Windows `.exe` produced by CI, not yet run on real Windows | **SQLite by default** (`orchestrator.db`); **PostgreSQL opt-in** via `DATABASE_URL=postgresql://...` |
...
- **Not run on a real Windows machine, at all.** Every verification above — the packaged executables, the sidecar auto-connect, the forceful-kill orphan-detection watchdog — was built and tested on Linux inside this session's sandbox. There is no Windows hardware available here. The Windows CI job was written carefully to mirror the working Linux path, but it has not been triggered and watched succeed, and the actual `.exe` outputs have not been downloaded and run by a human on Windows. This is the single largest remaining risk — see WP-31/WP-32's own sections for the specific unknowns (PyInstaller Windows quirks, antivirus false-positives on unsigned executables, Windows process-model differences for the orphan watchdog).
...
### WP-31 — One-Click Local Setup & Packaging (Windows-first) — 🟡 `gd-0.2.x` · Linux-validated, Windows CI added but not run on real hardware
...
- **What's built and verified (2026-07-01, this session — on Linux; see caveat below):**
  ...
  6. **`.github/workflows/build-windows.yml` extended** — added a `windows-latest` build leg (not a new workflow) that installs Python 3.11, runs both PyInstaller specs, stages the orchestrator as the Tauri sidecar binary, and uploads both executables as build artifacts (30-day retention) alongside the existing NSIS/WiX console installers.
...
- **⚠️ Honest caveat — not yet run on a real Windows machine.** Everything above was built and verified on Linux inside this session's sandbox (no Windows hardware available here). The Windows CI job (`build-windows.yml`) was written to mirror the already-working Linux build script step-for-step, and the CI workflow syntax/steps were reviewed carefully, but **it has not actually been triggered and watched succeed on a `windows-latest` runner as part of this work** — that verification is still open. The `.ps1` script is likewise unverified on real Windows. This is the single biggest remaining risk in WP-31: PyInstaller's Windows behavior (path separators, `.exe` extension handling, antivirus false-positives on freshly-built unsigned executables) can differ from Linux in ways a Linux-only spike cannot catch.
- **Remaining open steps (not done this session):**
  1. Actually run the Windows CI job and confirm the `.exe` outputs work when downloaded and double-clicked on a real Windows machine.
  2. Ollama detection & guidance in the desktop happy path (detect a local Ollama; if absent, link the user to install it).
...
- **Files:** `console/src-tauri/src/lib.rs` (rewritten), `console/src-tauri/Cargo.toml` (+`tauri-plugin-shell`, `+tauri-plugin-single-instance`, `+tokio` net/io-util/time features), `console/src-tauri/tauri.conf.json` (`bundle.externalBin`), `console/src/lib/stores/orchestrator.ts` (new), `console/src/lib/stores/auth.ts` (`getOrCreatePubkey`), `console/src/lib/components/ConnectDialog.svelte`, `orchestrator/packaging/entry.py` (orphan watchdog), `orchestrator/requirements.txt` (+`psutil`).
...
*Last updated: 2026-07-01 (WP-31 + WP-32 landed on top of WP-30's SQLite backend: the orchestrator and agent are now packaged as self-contained executables (PyInstaller, one-command `scripts/build-desktop.sh`/`.ps1`), the orchestrator auto-generates its JWT secret on first run, and the Tauri Console auto-spawns and auto-connects to a local orchestrator sidecar with zero manual steps — verified end-to-end against the real production `tauri build` binary via screenshot, including a forceful-kill orphan-detection watchdog that survives `SIGKILL`. All of this is Linux-validated only: no Windows hardware was available this session, so the Windows CI job that was added has not been watched succeed, and the `.ps1` script is unverified — that is now the single largest open gate on the desktop-first push, called out explicitly rather than assumed to work by extension from Linux.)*
