...
**Stack:** Agent Runtime — Python + FastAPI; Rust for security-critical paths · Manager Console — Tauri v2 + Svelte 5 + Tailwind (SteloPTC patterns) · Orchestrator — FastAPI + PostgreSQL (Docker Compose) · Transport — WSS over TLS · Inference — Ollama local-first · Containers — Docker multi-arch (CPU + CUDA)
...
- Circuit-breaker / retry discipline: 2 s / 4 s / 8 s / 16 s exponential backoff applied to all persistent connections
- Chart.js analytics: same visual language (line/bar/pie), tooltip format, and CSV/JSON export — embedded directly in the console
- Conversation engine and message rendering: embedded in the console's Agent Detail view — not reimplemented
- 12 agent role templates: extended with `jurisdiction` and `availability` metadata for distributed routing
- CDN SRI-hash validation discipline: applied here to Docker image layer integrity verification in CI
...
**Companion document:** `GruperDistributedSpec.md` — architecture diagrams, component specs, data model schemas, security threat table, and open questions (OQ-1…OQ-5).
...
- **Steps:**
  ...
  3. Map **Gruper core's per-agent config schema** (model, temperature, top-p, top-k, repeat penalty, max tokens, context length, role template) to the distributed task input schema — the formal bridge between core's local parameter model and the remote execution contract.
...
- **Goal:** Runnable Docker Compose stack (PostgreSQL + FastAPI) accepting agent registration and heartbeat; no task dispatch yet.
- **Steps:**
  1. `docker-compose.yml`: PostgreSQL 16 + FastAPI; all config via environment variables; no hardcoded credentials.
  ...
  4. JWT middleware; ed25519 keypair support stubbed (fully implemented at WP-07).
...
- **Exit gate:** `docker compose up` on a clean machine; mock agent registers, heartbeats, appears in `GET /agents`; all smoke tests pass in CI.
...
- **Goal:** Desktop agent service (Linux-first) that dials out to the orchestrator, executes tasks against local Ollama using **Gruper core's API shape and parameter conventions**, and streams results back.
- **Steps:**
  ...
  3. Task execution: receive task JSON → validate authority → call local Ollama (`/api/generate` or `/api/chat`, same parameter conventions as Gruper core) → stream progress and result over WSS.
  4. Offline queue: SQLite per-agent; drain on reconnect with exponential backoff.
  5. **Circuit-breaker:** 3 consecutive task failures → mark degraded, signal orchestrator to pause routing — mirrors **Gruper core's agent auto-disable pattern**.
  6. Heartbeat loop (30 s); graceful shutdown with in-flight task checkpoint.
  7. Systemd unit (Linux); launchd plist stub (macOS, activated at WP-15).
...
- **Exit gate:** Agent behind consumer NAT connects to VPS-hosted orchestrator; receives pushed task; calls local Ollama; streams result back — over the public internet relay path, not LAN-direct.
...
- **Steps:**
  ...
  6. Append to `events` table on every state transition (hash chain added at WP-17).
...
- **Goal:** Tauri v2 + Svelte 5 console scaffold with fleet view stub, task composer, and result view — embedding **Gruper core's conversation UI and Chart.js analytics** rather than reimplementing them.
- **Steps:**
  1. Tauri v2 + Svelte 5 + Tailwind scaffold; locked-down CSP in `tauri.conf.json` from commit one (SteloPTC pattern).
  ...
  4. **Task composer:** task input + model selector; UX mirrors **Gruper core's task-input conventions**.
  5. **Result view:** embed **Gruper core's conversation message rendering** (round-based display, markdown, glassmorphism styling) — not reimplemented.
  6. **Per-agent analytics tab:** embed **Gruper core's Chart.js response-time line chart** scoped to that agent's history; same visual language and CSV/JSON export as core.
...
- **Goal:** Prove the outbound-relay model stable over the public internet before sharing complexity is layered on.
- **Steps:**
  1. Orchestrator on VPS (Docker Compose); agent on workstation behind consumer NAT — no port forwarding.
  2. Submit 20 tasks from console; measure dispatch overhead (target < 10 s excluding model execution; SC-2 baseline).
...
- **Exit gate:** SC-5 met for single-owner case. SC-2 baseline documented. Relay model proven before WP-07 sharing is built on top of it.
...
- **Steps:**
  ...
  2. Agent registration: `owner_id` bound cryptographically — owner signs registration payload; orchestrator records public key as identity anchor.
  ...
  5. Migration to multi-tenant schema; `gd-0.2` single-owner data migrated cleanly.
  6. Integration tests: two users; no namespace bleed; each sees only their agents.
...
- **Steps:**
  ...
  2. `POST /tokens`: sign with owner key; return compact portable string (JWT or biscuit-style).
  3. `DELETE /tokens/{id}`: set `revoked_at`; stop dispatch immediately; signal in-flight task abort.
  4. Token verification middleware on **every dispatch**: scope, quota, time-window, data-class, revocation — no exceptions.
  5. Per-grantee quota enforcement at enqueue time: max concurrent tasks, RAM per task, wall time.
...
- **Exit gate:** Revocation takes effect within one dispatch cycle. Scope violations rejected by orchestrator before reaching the agent.

---

### WP-09 — Agent Runtime — Cross-Owner Dispatch — 🔲 `gd-0.3`

- **Goal:** Agent validates incoming dispatch authority locally, independent of orchestrator enforcement — defense in depth for cross-owner scenarios.
- **Steps:**
  ...
  4. Availability windows honored before accepting tasks from external grantees.
...
- **Exit gate:** Cross-location collaborator machine installs runtime, receives scoped token, executes dispatched task, returns result. Instant revoke tested live — no task accepted after revocation timestamp.
...
- **Steps:**
  ...
  4. **Agent Detail view:** for shared agents, only permitted actions are rendered — non-permitted actions not present in the UI.
...
- **Steps:**
  ...
  5. Console: delegation chain in task detail view; aggregated result in parent task pane.
  6. Adversarial test: scope escalation attempt rejected by orchestrator.
- **Exit gate:** Manager agent dispatches sub-tasks to ≥ 2 worker agents (one owned, one cross-owner shared); full delegation chain in audit log; escalation attempt rejected.
...
- **Steps:**
  ...
  7. **Image layer digests pinned in Terraform module** — mirrors **Gruper core's CDN SRI-hash CI validation** applied to container integrity; CI fails on digest mismatch.
...
- **Steps:**
  1. Terraform module: spot fleet for `g4dn.xlarge` / `g5.xlarge` (GPU) and `t3.medium` (CPU); `--instance-interruption-behavior=terminate`.
  2. User Data: runs WP-12 `docker run` one-liner; reads secrets from AWS Secrets Manager.
  3. **Hard budget cap:** `POST /pools` accepts `max_spend_usd`; orchestrator refuses dispatch if projected pool cost would breach cap — enforced at enqueue, before any instance action.
...
- **Exit gate:** 2-node spot pool launched; batch of tasks completes; cost cap halts dispatch at threshold; idle auto-terminate fires within 5 min of queue drain. Spend does not exceed cap under any tested scenario.
...
- **Steps:**
  ...
  3. Bounds: `min_agents` / `max_agents` per pool; budget cap enforced before scale-out.
  4. Scale-down: drain gracefully; terminate only after current task completes.
  5. Cost and utilization dashboard: per-pool spend vs cap, queue depth trend — extends **Gruper core's Chart.js analytics** with cloud cost dimension; same export format.
...
- **Goal:** Every task on every agent type runs in an isolated per-task sandbox. Desktop and container containment demonstrably equivalent — exit gate condition, not a best-effort goal.
- **Steps:**
  1. **Linux desktop:** Firejail — isolated tmpfs, dropped capabilities, seccomp filter, empty netns with egress allow-list, cgroup CPU/memory/wall-time.
  ...
  5. **Equivalence validation suite:** same task on all four environments; filesystem isolation, network egress, resource limits verified identically; written results documented.
...
- **Exit gate:** Written equivalence report signed off. All four environments pass validation. Cross-user sharing blocked in console until this WP closes.

---

### WP-16 — E2E Payload Encryption — 🔲 `gd-0.5`

- **Goal:** E2E payload encryption is a **first-class security requirement** — the orchestrator routes task payloads it cannot read.
- **Steps:**
  ...
  2. Submitter (console): encrypt task payload to **target agent's X25519 public key** (X25519 ECDH + ChaCha20-Poly1305 AEAD) before submitting.
  ...
  5. Key rotation: agent regenerates X25519 keypair on demand; orchestrator invalidates old reference on re-registration.
  6. Integration test: inspect orchestrator DB — payload column contains ciphertext, not plaintext.
  7. Simulated compromised-orchestrator test: full DB read access; task payload content remains confidential.
...
- **Goal:** Audit event stream is append-only and tamper-evident — verifiable compliance record for regulated environments and run attribution.
- **Steps:**
  ...
  5. Standalone chain-verification CLI: downloads full chain, verifies hash continuity — for compliance audits independent of the console.
  6. Redaction: sensitive payload content appears only as `payload_hash`; no plaintext in event record.
- **Exit gate:** Chain verifies end-to-end for 500-event dataset. Tampering any single field causes verification failure at that event and all subsequent. Compliance JSON export tested.
...
- **Steps:**
  ...
  2. Threat model verification: every row in the companion spec's threat table verified by implemented controls — not just documented, but tested.
...
- **Steps:**
  ...
  4. Console: "best match" option in task composer alongside explicit assignment; shows matched agent(s) and routing rationale before dispatch.
...
- **Steps:**
  1. `Crew` model: DAG of `Task` nodes with agent target (explicit or capability-match), input sources, output handling, and timeout.
  2. Visual editor in console: drag-and-drop node graph; nodes = agent task assignments; edges = data flow.
  3. YAML/JSON import/export for crew definitions — version-controllable and shareable.
  4. Crew execution engine in orchestrator: resolves DAG, dispatches as upstream deps complete, aggregates outputs.
  5. Crew result view: extends **Gruper core's conversation UI** — multi-step execution as a conversation thread, one message per agent step, consistent with core's round-based display.
  6. n8n integration: crew launchable from n8n webhook; crew steps can call n8n workflows as tool calls.
- **Exit gate:** 3-node crew (≥ 1 owned + ≥ 1 cross-owner shared agent) completes a DAG task end-to-end; aggregated result displayed in the console crew result view.

---

### WP-21 — Extended Fleet Analytics & Monitoring — 🔲 `gd-0.7`

- **Goal:** Fleet-wide monitoring dashboard extending **Gruper core's Chart.js visual language** and export format to distributed fleet metrics and cloud cost tracking.
- **Steps:**
  1. Fleet metrics: per-agent success rate, response latency by agent/location, task throughput, queue depth, utilization heatmap.
  ...
  4. **Identical to core:** Chart.js line/bar/pie with same color scheme, tooltip format, and CSV/JSON export — recognizable to Gruper core users immediately.
  5. Threshold alerting: agent offline, queue depth, or cost exceeds configured threshold → console toast + optional n8n webhook.
- **Exit gate:** Dashboard shows live data for ≥ 3 agents (LAN + shared + cloud). All charts export to CSV/JSON. Alert fires correctly on simulated offline and cost-threshold events.

---

### WP-22 — n8n Bidirectional Integration — 🔲 `gd-0.8`

- **Goal:** Agents callable from n8n workflows; agent tasks can trigger n8n flows — deterministic automation and AI reasoning operating as peers.
- **Steps:**
  1. **n8n → agent:** HTTP node or custom n8n community node posts to `POST /tasks`; receives result via webhook callback or polling.
  2. **Agent → n8n:** `n8n_webhook` tool in agent runtime; calls configured n8n workflow URL; receives HTTP response as tool result.
  3. **Crew → n8n:** crew node type wrapping n8n workflow as a DAG step.
  4. Authentication: scoped orchestrator API token per n8n integration (not admin credential).
...
- **Steps:**
  1. **Beta participants:** 2–3 collaborators across distinct use cases (≥ 1 regulated/compliance-sensitive); confirmed before `gd-0.6` begins.
  2. **Install guide:** all three agent paths (desktop Linux, desktop Windows, Docker); QR onboarding; < 5 min for a non-technical user.
  3. **Sharing setup guide:** mint token, configure scope, share, monitor usage, revoke — no command-line assumed.
  4. **Ops runbook:** orchestrator deploy, backup/restore, TLS renewal, log rotation, cost-cap monitoring.
  5. **Security posture summary (1-page):** for collaborators — what the owner can/cannot see, data-class routing, how to revoke.
  6. **SC-1…SC-7 verification checklist:** run against beta environment with real participants; document evidence per criterion.
  7. No critical bugs open at handoff.
- **Exit gate:** All SC-1…SC-7 documented as met for real beta users. Documentation reviewed by ≥ 1 beta participant. No critical bugs open. Ready to assess v1.0.
...
| WP | Item | Notes |
|----|------|-------|
| WP-24 | **Pattern B — Federated per-user orchestrators** | Each user self-hosts; sharing authorizes cross-orchestrator dispatch; agent multi-homing. Token and data model in WP-08 must not preclude this additive change. |
| WP-25 | **Direct P2P channels (WebRTC / QUIC)** | Orchestrator brokers ICE; large artifacts transferred peer-to-peer; automatic relay fallback. Added only after relay path proven in production. |
| WP-26 | **Mobile / PWA console** | Read-only fleet status and approval actions. Console API designed from `gd-0.2` to not preclude this. |
| WP-27 | **Predictive cloud instance pre-warming** | Pre-launch spot instances from historical task arrival patterns. Requires stable utilization baseline. |
| WP-28 | **Cross-machine crews with full scope inheritance** | Full DAG execution across ownership tiers with automatic scope propagation. Requires WP-20 as base. |
...
**Permanently out of scope:** public trustless compute marketplace; blockchain token incentives; anyone-can-join compute grid; server-side Gruper core (core stays client-only, single-file, and standalone).
...
| Item | Severity | Notes |
|------|----------|-------|
| OQ-1 through OQ-5 unresolved | **Blocking** | Five open architecture questions must be resolved before WP-01 closes; all downstream WPs depend on them. See companion spec §12.2. |
| Desktop sandboxing (Linux only until WP-15) | **High** | Windows and macOS per-task sandbox not implemented until `gd-0.5`. Cross-owner sharing on Windows/macOS desktop agents blocked until WP-15 and WP-18 close. |
| `events` table lacks hash chain until WP-17 | **Medium** | Event append active from WP-02; tamper-evidence absent until `gd-0.5`. Compliance-sensitive deployments must not go live before WP-17 closes. |
| Python agent runtime (memory-unsafe) | **Medium** | Acceptable for prototyping. Rust port of sandbox and comms components planned as load and security review demand it; no fixed timeline. |
| Single-orchestrator SPOF | **Medium** | Failover interface in data model from WP-01; full federation (Pattern B) deferred to `gd-1.x`. Agent offline queue (WP-03) mitigates partial impact. |
...
*Companion document: `GruperDistributedSpec.md` — architecture diagrams, data models, wire schemas, security threat table, open questions (OQ-1…OQ-5).*
