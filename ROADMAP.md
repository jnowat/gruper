...
- **Steps:**
  ...
  8. ✅ CI build: `console/package-lock.json` committed and **verified `npm ci`-coherent** (regenerated with npm 11 to dodge npm 10.9's lockfile-generation bug; npm 10 *and* npm 11 both accept it). Readiness check passes, so `tauri-action` runs the real NSIS/WiX build on push to `main` and on `v*` tags. Workflow pins npm to 11 and logs node/npm versions for reproducibility. Frontend (`npm ci && npm run build && npm run check`) verified green on Linux. **Windows build fix:** the Tauri v2 library-naming bug (`gruper_console_lib` unresolved — `Cargo.toml` had no `[lib]` section, so the library defaulted to the package/bin name `gruper_console`) is fixed with an explicit `[lib] name = "gruper_console_lib"` (`crate-type = ["staticlib", "cdylib", "rlib"]`); the placeholder bundle icons (RGB PNGs that made `tauri::generate_context!` panic with *"icon … is not RGBA"*) are regenerated as valid RGBA via `tauri icon`. With both fixes the full native Tauri shell now `cargo build`s green on Linux (deps + `lib.rs`/`main.rs` link + `generate_context!` asset validation all pass); the Windows MSVC bundle leg (`.exe`/`.msi`) runs on the GitHub Windows runner. `Cargo.lock` committed for reproducible CI builds.
